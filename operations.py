...
class Hola:
    ...
    def report(self):
        df = self.get_operations_df(account_id=creds.account_id)
        df = df[df['otype'] == OperationType.OPERATION_TYPE_BROKER_FEE]
        #print(df.head(100))



        df_tax = self.get_operations_df(account_id=creds.account_id)
        df_tax = df_tax[df_tax['otype'].isin([
            OperationType.OPERATION_TYPE_TAX,
            OperationType.OPERATION_TYPE_BOND_TAX,
            OperationType.OPERATION_TYPE_DIVIDEND_TAX,
        ])]
        #print(df_tax.head(100))
        print('comission', df['payment'].sum())
        print('Taxes:', df_tax['payment'].sum())
